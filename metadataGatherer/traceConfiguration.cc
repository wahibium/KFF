...
TraceConfiguration::TraceConfiguration()
{
	...
	try
	{
		std::ifstream stream("configure.kff");
	...
	if(memory)
	{
		...
		kff::addTraceGenerator(_memoryTraceGenerator, true);			
	}
	...
	if(sharedComputation)
	{
		...
		kff::addTraceGenerator(_sharedComputationGenerator, true);
	}
	...
	if(branch)
	{
		...
		kff::addTraceGenerator(_branchTraceGenerator, true);
	}
	...
	if(parallelism)
	{
		...
		kff::addTraceGenerator(_parallelismTraceGenerator, true);
	}
	...
	if(instruction)
	{
		...
		kff::addTraceGenerator(_instructionTraceGenerator, true);
	}
	...
	if(cacheSimulator.enabled)
	{
		...
		kff::addTraceGenerator(_cacheSimulator, true);	
	}
	...
	if(warpSynchronous.enabled) 
	{
		...
		kff::addTraceGenerator(_warpSynchronous, true);
	}
	...
	if(performanceBound.enabled) 
	{
		...
		kff::addTraceGenerator(_performanceBound, true);
	}
	...
	if(convergence.enabled) 
	{
		...
		kff::addTraceGenerator(_convergence, true);
	}
	...
	if (loadBalance)
	{
		...
		kff::addTraceGenerator(_loadBalance, true);
	}
	...
	if (controlFlowVisualizer.enabled)
	{
		...
		_controlFlowVisualizer.allInstructions =
			controlFlowVisualizer.allInstructions;
		kff::addTraceGenerator(_controlFlowVisualizer, true);
	}
	...
	if (temporalSIMT.enabled) {
		...
		kff::addTraceGenerator(_temporalSIMTGenerator);
	}
	...
	if (basicBlockCount)
	{
		...
		kff::addTraceGenerator(_basicBlockCountGenerator, true);
	}
	...
	if(x86trace)
	{
		...
		kff::addTraceGenerator(_x86TraceGenerator, true);
	}
	
	if (activityFactor) {
		report("Creating Activity Factor Generator");
		kff::addTraceGenerator(_activityFactorGenerator, true);
	}
}
