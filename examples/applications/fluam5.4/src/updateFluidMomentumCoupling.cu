...
__global__ void updateFluidMomentumCoupling(double* vxGPU, 
					    double* vyGPU, 
					    double* vzGPU,
					    double* densityGPU,
  ...
  vxGPU[i] += (fx * 2. / (densityGPU[i] + densityGPU[vecino3])) ;
  vyGPU[i] += (fy * 2. / (densityGPU[i] + densityGPU[vecino4])) ;
  vzGPU[i] += (fz * 2. / (densityGPU[i] + densityGPU[vecino5])) ;
