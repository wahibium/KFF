// Filename: saveFunctionsSchemeParticlesWall.cu
//
...
bool saveFunctionsSchemeParticlesWall(int index, long long step){
  ...
  if(index==0){
    ...
    if(!temperatureParticlesWall(index)) return 0;
    if(setparticles)
      if(!saveParticles(index,step)) return 0;
    //if(!hydroAnalysisParticlesWall(0)) return 0;
    //if(!hydroAnalysisParticlesWall(1)) return 0;//////
  ...
  //Close save functions
    ...
    if(setparticles)
      if(!saveParticles(index,step)) return 0;
    //if(!hydroAnalysisParticlesWall(2)) return 0;
    //if(!covarianceKernel(index)) return 0;
