// Filename: createCellsStokesLimitGPU.cu
//
...
bool createCellsStokesLimitGPU(){
  ...
  double fact1 = sqrt((4.*temperature*shearviscosity)/(cVolume*dt));
  double fact4 = sqrt((2.*temperature*shearviscosity)/(cVolume*dt));
  double fact5 = sqrt(1./(dt*cVolume));
  double fact2 = sqrt(temperature * extraMobility * dt);
  ...
  cutilSafeCall(cudaMemcpyToSymbol(setExtraMobilityGPU,&setExtraMobility,sizeof(bool)));
  cutilSafeCall(cudaMemcpyToSymbol(extraMobilityGPU,&extraMobility,sizeof(double)));
