// Filename: schemeFreeEnergyCompressibleParticles.cu
//
...
bool schemeSemiImplicitCompressibleParticles(){
  ...
  if(!createCellsSemiImplicitCompressibleParticlesGPU()) return 0;
  ...
  if(!saveFunctionsSchemeSemiImplicitCompressibleParticles(0,0)) return 0;




  //Run the simulation
  if(!runSchemeSemiImplicitCompressibleParticles()) return 0;
     



  //Close save functions
  if(!saveFunctionsSchemeSemiImplicitCompressibleParticles(2,0)) return 0;
  ...
  //Free Memory GPU
  if(!freeCellsSemiImplicitCompressibleParticlesGPU()) return 0;
  ...
  //Free memory
  if(!freeMemorySemiImplicitCompressibleParticles()) return 0;
