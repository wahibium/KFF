// Filename: kernelDpFreeEnergyCompressibleParticles.cu
//
...
__global__ void kernelDpFreeEnergyCompressibleParticles(double* densityGPU, 
							double* densityGPU2,
							double* vxGPU, 
							double* vyGPU,
							double* vzGPU,
							...
							double* omegaGPU,
							long long step,
							double RK1, double RK2, double RK3){
  ...
  //Include external pressure
  /*int kz = i / (mxmytGPU);
  double dp0 = 0.005;
  //double frequency=0.0392540992624731724;//mz=64
  double frequency=0.078413391828196923;//mz=32
  //double frequency=0.156069730967592385;//mz=16
  ...
  if(kz==0){
    pressure  += dp0 * sin(frequency*time);
    pressure3 += dp0 * sin(frequency*time);
    pressure4 += dp0 * sin(frequency*time);
  }
  else if(kz==(mzGPU-1)){
    pressure5 += dp0 * sin(frequency*time);
    }*/
  ...
  //Particle extra pressure
  pressure  += omegaGPU[i];
  pressure3 += omegaGPU[vecino3];
  pressure4 += omegaGPU[vecino4];
  pressure5 += omegaGPU[vecino5];
