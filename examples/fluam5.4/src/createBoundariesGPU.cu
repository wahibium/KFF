// Filename: createboundariesGPU.cu
//
...
bool createBoundariesGPU(){
  ...
  cudaMemcpy(countparticlesincellZ,aux,ncellst*sizeof(int),cudaMemcpyHostToDevice);

  cudaMalloc((void**)&partincellX,maxNumberPartInCell*ncellst*sizeof(int));
  cudaMalloc((void**)&partincellY,maxNumberPartInCell*ncellst*sizeof(int));
  cudaMalloc((void**)&partincellZ,maxNumberPartInCell*ncellst*sizeof(int));
  ...
  cutilSafeCall( cudaBindTexture(0,texCountParticlesInCellX,countparticlesincellX,ncellst*sizeof(int)));
  ...
  cutilSafeCall( cudaBindTexture(0,texCountParticlesInCellY,countparticlesincellY,ncellst*sizeof(int)));
  ...
  cutilSafeCall( cudaBindTexture(0,texCountParticlesInCellZ,countparticlesincellZ,ncellst*sizeof(int)));
  //texPartInCellX;
  ...
  cutilSafeCall( cudaBindTexture(0,texPartInCellX,partincellX,maxNumberPartInCell*ncellst*sizeof(int)));
  //texPartInCellY;
  ...
  cutilSafeCall( cudaBindTexture(0,texPartInCellY,partincellY,maxNumberPartInCell*ncellst*sizeof(int)));
  //texPartInCellZ;
  ...
  cutilSafeCall( cudaBindTexture(0,texPartInCellZ,partincellZ,maxNumberPartInCell*ncellst*sizeof(int)));
  ...
  if(setparticles){
    ...
    if(particlesWall){
      myPart = int((ly+2*ly/my)/cutoff) + 2;
      if(myPart < 5) myPart = 5;
    }
    ...
    cutilSafeCall( cudaBindTexture(0,texCountParticlesInCellNonBonded,countPartInCellNonBonded,
    ...
    initializeNeighbors<<<block,128>>>(neighbor1GPU,
				       neighbor2GPU,
				       neighbor3GPU,
				       neighbor4GPU,
				       neighborpxpyGPU,
				       ...
				       neighbormxmypzGPU,
				       neighbormxmymzGPU);
    initializeNeighbors2<<<block,128>>>(neighbor0GPU,
					neighbor1GPU,
					neighbor2GPU,
					neighbor3GPU,
					neighbor4GPU,
  ...
  initParticlesInCell<<<1,1>>>(partincellX,
			       partincellY,
			       partincellZ,
			       countparticlesincellX,
			       countparticlesincellY,
			       countparticlesincellZ,
			       countPartInCellNonBonded,
			       partInCellNonBonded,pc);


  if(freeEnergyCompressibleParticles || momentumCoupling){

    cudaMemcpyToSymbol(omega0GPU,&omega0,sizeof(double));
    cudaMalloc((void**)&fboundaryOmega,27*(nboundary+np)*sizeof(double));
    cutilSafeCall(cudaMalloc((void**)&omegaGPU,ncellst*sizeof(double)));

    cutilSafeCall(cudaMalloc((void**)&countparticlesincell,ncellst*sizeof(int)));
    for(int i=0;i<ncellst;i++) aux[i] = 0;
    cudaMemcpy(countparticlesincell,aux,ncellst*sizeof(int),cudaMemcpyHostToDevice);
    cudaMalloc((void**)&partincell,maxNumberPartInCell*ncellst*sizeof(int));
    initParticlesInCellOmega<<<1,1>>>(partincell,
                                      countparticlesincell,
                                      pc);
