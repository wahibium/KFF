// Filename: schemeContinuousGradient.cu
//
...
bool freeMemoryContinuousGradient();
...
bool schemeContinuousGradient(){
  ...
  if(!initializeFluidBinaryMixture()) return 0;
  ...
  //Run the simulation
  if(!runSchemeContinuousGradient()) return 0;
  ...
  //Free memory
  if(!freeMemoryContinuousGradient()) return 0;
