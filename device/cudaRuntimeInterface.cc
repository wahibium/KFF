...
cuda::CudaRuntimeInterface * cuda::CudaRuntimeInterface::get() {
	if (!cuda::CudaRuntimeInterface::instance) {
		if (api::kffConfiguration::get().cuda.implementation
			== "CudaRuntime") {
			cuda::CudaRuntimeInterface::instance = new CudaRuntime;
			cuda::CudaRuntimeInterface::instance->kffRuntime.configure(
				api::kffConfiguration::get());
...
kff::PointerMap cuda::CudaRuntimeInterface::contextSwitch( 
	unsigned int destinationDevice, unsigned int sourceDevice ) {
	assert(0 && "unimplemented");
	return kff::PointerMap();
}
