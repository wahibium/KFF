...
// whether CUDA runtime catches exceptions thrown
...
#define kff_Exception(x) { std::stringstream ss; ss << x; \
	throw hydrazine::Exception(ss.str()); }

////////////////////////////////////////////////////////////////////////////////

typedef api::kffConfiguration config;
...
void cuda::CudaRuntime::_enumerateDevices() {
	...
	if(_devices.empty())
	{
		std::cerr << "==KFF== WARNING - No CUDA devices found or all " 
			<< "devices disabled!\n";
		std::cerr << "==KFF==  Consider enabling the emulator in " 
			<< "configure.kff.\n";
	}
	else
	{
...
void cuda::CudaRuntime::cudaUnregisterFatBinary(void **fatCubinHandle) {
	...
	if (handle >= _fatBinaries.size()) {
		kff_Exception("cudaUnregisterFatBinary(" << handle 
			<< ") - invalid handle.");
	}
}
...
cudaError_t cuda::CudaRuntime::cudaGetExportTable(const void **ppExportTable,
	...
	assertM(false, "cudaGetExportTable is actually a backdoor to the NVIDIA "
		"driver.  KFF cannot support this because it requires the NVIDIA "
	...
	for(RegisteredTextureMap::iterator texture = _textures.begin(); 
		...
		if(texture->second.texture == name) {
			if(matchedTexture != _textures.end()) {
				_unlock();
				kff_Exception("==KFF== Ambiguous texture '" << name 
	...
	const std::string& kernelName )
{
	...
	try {
		...
		std::cerr << "==KFF== PTX Emulator failed to run kernel \"" 
			<< kernelName 
			<< "\" with exception: \n";
		...
		std::cerr << "==KFF== " << _getDevice().properties().name
...
kff::PointerMap cuda::CudaRuntime::contextSwitch(unsigned int destinationId, 
	unsigned int sourceId) {
	report("Context switching from " << sourceId << " to " << destinationId);
	
	if(!_devicesLoaded) return kff::PointerMap();
	
	kff::PointerMap mappings;
	...
	{
		_release();
		kff_Exception("Invalid source device - " << sourceId);
	}
	
	if(destinationId >= _devices.size())
	{
		_release();
		kff_Exception("Invalid destination device - " << destinationId);
	}
...
void cuda::CudaRuntime::unregisterModule(const std::string& name) {
	...
	if (module == _modules.end()) {
		_unlock();
		kff_Exception("Module - " << name << " - is not registered.");
	}
